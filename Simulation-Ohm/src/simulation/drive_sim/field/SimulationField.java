...
public class SimulationField {
	...
	public SimulationField(int width, int height) {
		...
		visibility = DriverStation.getVisibility(driverStation, myAlliance).getScaledCopy(width, height).getFlippedCopy(
				myAlliance == Alliance.RED, false);
		try {
	...
	public enum DriverStation {
		...
		public static Image getVisibility(DriverStation ds, Alliance alliance) {
			Image image = null;
			try {
				image = new Image(getVisibilityString(ds, alliance));
			} catch (SlickException e) {
				e.printStackTrace();
			}
			return image;
